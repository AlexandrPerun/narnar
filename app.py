...
def main(drawing_mode, color, point_display_radius, arrows, arrows_thickness, im1, im2, realtime_update):
    if im1:
        im1 = Image.open(im1)
...
def load_layout():
    ...
    arrows_thickness = st.sidebar.slider("Arrows thickness: ", 1, 5, 2) if arrows else 0

    im1 = st.sidebar.file_uploader("First image:", type=["png", "jpg"])
    im2 = st.sidebar.file_uploader("Second image:", type=["png", "jpg"])
    realtime_update = st.sidebar.checkbox("Update in realtime", True)

    main(drawing_mode, color, point_display_radius, arrows, arrows_thickness, im1, im2, realtime_update)
